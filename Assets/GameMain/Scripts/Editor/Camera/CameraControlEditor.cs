...
namespace Flower
{
    ...
    public class CameraRigEditor : UnityEditor.Editor
    {
        ...
        void OnSceneGUI()
        {
            ...
            var fmh_42_69_638471540137250569 = Quaternion.LookRotation(Vector3.up); Vector3 bottom = Handles.FreeMoveHandle(bottomPosition, size, snap,
                                                    Handles.RectangleHandleCap);
            var fmh_44_63_638471540137280771 = Quaternion.LookRotation(Vector3.up); Vector3 top = Handles.FreeMoveHandle(topPosition, size, snap,
                                                 Handles.RectangleHandleCap);
            var fmh_46_65_638471540137283603 = Quaternion.LookRotation(Vector3.up); Vector3 left = Handles.FreeMoveHandle(leftPosition, size, snap,
                                                  Handles.RectangleHandleCap);
            var fmh_48_67_638471540137285905 = Quaternion.LookRotation(Vector3.up); Vector3 right = Handles.FreeMoveHandle(rightPosition, size, snap,
                                                   Handles.RectangleHandleCap);
